...
SCSAPI_VOID handle_frame_start(const scs_event_t UNUSED(event), const void* const UNUSED(event_info), const scs_context_t context) {
	...
	state << "Near " << telemetry->position.closestCity << " (" << telemetry->position.country << ")";
	...
	set_activity(!telemetry->position.avalible || telemetry->paused ? "In Main Menu" : on_job ? details.str() : u8"🚚 Without cargo 💨",
